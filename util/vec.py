...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, {key: u[key] + v[key] for key in u.D})
...
def dot(u,v):
    ...
    return sum([u[key] * v[key] for key in set(u.f.keys() | v.f.keys())])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {key : v[key] * alpha for key in v.D})
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {key : v[key] * -1 for key in v.D})
