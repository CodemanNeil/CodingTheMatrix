...
def equal(A, B):
    ...
    assert A.D == B.D
    isEqual = True
    for row, col in set(A.f.keys() | B.f.keys()):
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat((A.D[0], A.D[1]), {key:A[key] + B[key] for key in set(A.f.keys() | B.f.keys())})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat((M.D[0], M.D[1]), {key:x * M[key] for key in M.f.keys()})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(key[1], key[0]):M[key] for key in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    # return Vec(M.D[1], {col: v * Vec(M.D[0], {row:M[row, col] for row in M.D[0]}) for col in M.D[1]})
    result = Vec(M.D[1], {})
    for i,j in M.f.keys():
        result[j] += M[i,j] * v[i]
    return result
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    # return Vec(M.D[0], {row: v * Vec(M.D[1], {col:M[row, col] for col in M.D[1]}) for row in M.D[0]})
    result = Vec(M.D[0], {})
    for i,j in M.f.keys():
        result[i] += M[i,j] * v[j]
    return result
...
def matrix_matrix_mul(A, B):
    ...
    colsAB = {}
    for colB in B.D[1]:
        colsAB[colB] = A * Vec(B.D[0], {rowB: B[rowB, colB] for rowB in B.D[0]})
    return Mat((A.D[0], B.D[1]), {(r,c):colsAB[c][r] for c in colsAB.keys() for r in A.D[0]})
