...
list_sum_zip = [sum(tuple) for tuple in zip(A, B)]
...
square_dict = {key : key**2 for key in range(100)}
...
representation_dict = {x : [x // base**2, x // base, x % base] for x in digits}
...
listdict2dict = {names[key] : value for (key,value) in id2salary.items()}
...
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def dict2list(dct, keylist): return [dict[x] for x in keylist]
