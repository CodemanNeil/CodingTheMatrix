...
part_2_AB = [[5,1,0,2],[2,6,-4,1],[2,3,0,-4],[-2,3,4,0]]
part_2_BA = [[3,4,0,-2],[3,0,-4,2],[1,-4,6,2],[2,0,1,5]]

part_3_AB = [[1,0,5,2],[6,-4,2,1],[-4,0,2,3],[0,4,-2,3]]
part_3_BA = [[3,4,0,-2],[1,-4,6,2],[3,0,-4,-2],[3,4,0,-2]]
...
def lin_comb_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    result = Vec(M.D[0], {})
    for col_label, col_vec in mat2coldict(M).items():
        result += v[col_label] * col_vec
    return result
...
def lin_comb_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    result = Vec(M.D[1], {})
    for row_label, row_vec in mat2rowdict(M).items():
        result += v[row_label] * row_vec
    return result
...
def dot_product_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return Vec(M.D[0], {row_label: row_vec * v for row_label, row_vec in mat2rowdict(M).items()})
...
def dot_product_vec_mat_mult(v, M):
    ...
    Examples:
      ...
      '''
    assert(v.D == M.D[0])
    return Vec(M.D[1], {col_label: col_vec * v for col_label, col_vec in mat2coldict(M).items()})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    result_col_dict = {}
    for col_label, col_vec in mat2coldict(B).items():
        result_col_dict[col_label] = A * col_vec
    return coldict2mat(result_col_dict)
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    result_row_dict = {}
    for row_label, row_vec in mat2rowdict(A).items():
        result_row_dict[row_label] = row_vec * B
    return rowdict2mat(result_row_dict)
...
voting_list = list(f)
col_dict = {}
for country_vote_string in voting_list:
    country_vote = country_vote_string.split()
    country_name = country_vote[0]
    country_votes = country_vote[1:]
    num_votes = len(country_votes)
    vote_vec = Vec(set(range(num_votes)), {i:int(country_votes[i]) for i in range(num_votes)})
    col_dict[country_name] = vote_vec
A = coldict2mat(col_dict)
M = A.transpose() * A

sorted_pairs = sorted([(value,key) for key,value in M.f.items()])
# Provide a set consisting of two strings
most_opposed_pair_of_countries = sorted_pairs[0][1]
print(most_opposed_pair_of_countries)

# Provide a ten-element list of two-element sets of strings
most_opposed_10_pairs_of_countries = [sorted_pairs[i][1] for i in range(10)]
print(most_opposed_10_pairs_of_countries)

# Provide a set consisting of two strings
most_agreeing_pair_of_countries = sorted_pairs[-1][1]
print(most_agreeing_pair_of_countries)
...
def dictlist_helper(dlist, k):
    ...
    ['Apfel', 'manzana', 'pomme']
    '''
    return [dictionary[k] for dictionary in dlist]
