...
# RG should be the identity matrix (4x4)
print(R*G)
...
# HG should be a 3x4 matrix with all zeros
print(H*G)
...
def find_error(syndrome):
    ...
    Examples:
        ...
        True
    """
    # position is calculated by binary. 0 index so subtract 1
    error_position = sum(2**(2-i) for i in syndrome.f.keys() if syndrome[i] == one)
    if error_position == 0:
        return Vec({0,1,2,3,4,5,6}, {})
    else:
        return Vec({0,1,2,3,4,5,6}, {error_position-1:one})
...
def find_error_matrix(S):
    ...
    Example:
        ...
        True
    """
    return coldict2mat({col_label: find_error(col_vec) for col_label, col_vec in mat2coldict(S).items()})
