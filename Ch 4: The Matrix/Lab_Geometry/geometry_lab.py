...
## Constant
POS_DOMAIN_SET = {'x','y','u'}
COLOR_DOMAIN_SET = {'r','g','b'}
...
def identity():
    ...
    True
    '''
    return Mat((POS_DOMAIN_SET, POS_DOMAIN_SET), {(i, i): 1 for i in POS_DOMAIN_SET})
...
def translation(alpha,beta):
    ...
    True
    '''
    return Mat((POS_DOMAIN_SET, POS_DOMAIN_SET), {('x', 'x'): 1, ('y', 'y'): 1, ('u', 'u'): 1,
                                                  ('x','u'): alpha, ('y','u'): beta})
...
def scale(alpha, beta):
    ...
    True
    '''
    return Mat((POS_DOMAIN_SET, POS_DOMAIN_SET), {('x', 'x'): alpha, ('y', 'y'): beta, ('u', 'u'): 1})
...
def rotation(theta):
    ...
    True
    '''
    return Mat((POS_DOMAIN_SET, POS_DOMAIN_SET), {('x', 'x'): math.cos(theta), ('y', 'x'): math.sin(theta),
                                                  ('x','y'): -math.sin(theta), ('y','y'): math.cos(theta),
                                                  ('u','u'): 1})
...
def rotate_about(theta, x, y):
    ...
    >>> (rotate_about(math.pi/2, -1, 0)*Vec({'x','y','u'}, {'x':1, 'y':0, 'u':1}) - Vec({'x', 'y', 'u'},{'x': -1, 'y': 2, 'u': 1})).is_almost_zero()
    True
    '''
...
def scale_color(scale_r,scale_g,scale_b):
    ...
    True
    '''
    return Mat((COLOR_DOMAIN_SET, COLOR_DOMAIN_SET), {('r','r'): scale_r, ('g','g'): scale_g, ('b','b'): scale_b})
...
def grayscale():
    ...
    '''
    return rowdict2mat(Vec({'r','g','b'}, {'r': 77/256, 'g': 151/256, 'b': 28/256}))
...
if __name__ == '__main__':
    print(rotate_about(math.pi/2, -1, 0)*Vec({'x','y','u'}, {'x':1, 'y':0, 'u':1}))
    print(rotate_about(math.pi/2, -1, 0)*Vec({'x','y','u'}, {'x':1, 'y':0, 'u':1}))
