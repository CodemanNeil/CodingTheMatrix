...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    W = [x for x in S if x not in A]
    for w in W:
        T = list(S)
        T.append(z)
        if is_superfluous(T, T.index(w)):
            return w
