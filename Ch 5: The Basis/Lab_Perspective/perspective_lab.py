...
def make_nine_equations(corners):
    ...
    '''
    equations = []
    whiteboard_corners = [(0,0), (0,1), (1,0), (1,1)]
    for index, corner in enumerate(corners):
        whiteboard_corner = whiteboard_corners[index]
        equations.extend(make_equations(corner[0], corner[1], whiteboard_corner[0], whiteboard_corner[1]))
    equations.append(w)
    return equations
...
def mat_move2board(Y):
    ...
    <BLANKLINE>
    '''
    Y_cols = mat2coldict(Y)
    return coldict2mat([move2board(col) for col in Y_cols.values()])
